...
CREATE TABLE IF NOT EXISTS `domiciliation`.`contract` (
  ...
  `id_justificatif` INT NOT NULL,
  `id_option` INT NOT NULL,
  PRIMARY KEY (`id_contract`, `id_justificatif`),
...
CREATE TABLE IF NOT EXISTS `domiciliation`.`clients` (
  ...
  PRIMARY KEY (`id_client`, `id_responsable`, `id_contract`),
  UNIQUE INDEX `codeclient_UNIQUE` (`code_client` ASC),
  INDEX `fk_clients_responsablelegale1_idx` (`id_responsable` ASC),
  INDEX `fk_clients_contract1_idx` (`id_contract` ASC),
  INDEX `fk_clients_forme_juridique1_idx` (`id_forme` ASC),
...
CREATE TABLE IF NOT EXISTS `domiciliation`.`courrier` (
  ...
  `id_reexpedition` INT NOT NULL,
  `id_type_courrier` INT NOT NULL,
  `id_lettre` INT NOT NULL,
  PRIMARY KEY (`id_courrier`, `id_client`),
  INDEX `fk_courrier_clients1_idx` (`id_client` ASC),
  INDEX `fk_courrier_voiereexpedition1_idx` (`id_reexpedition` ASC),
  INDEX `fk_courrier_typecourrier1_idx` (`id_type_courrier` ASC),
  INDEX `fk_courrier_lettres_recommandees1_idx` (`id_lettre` ASC),
...
CREATE TABLE IF NOT EXISTS `domiciliation`.`user` (
  ...
  INDEX `fk_utilisateur_droit1_idx` (`id_droit` ASC),
